...
const AssignmentHelper = () => {
    ...
    // Alternative simplified function for text-only requests
    const handleTextOnlySubmit = async () => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:8000/ai/text-only', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json'
            ...
            if (!response.ok) {
                const errorText = await response.text();
                console.error('Text-only response error:', errorText);
                throw new Error(`HTTP ${response.status}: ${errorText}`);
            }

            const data = await response.json();
            console.log('Text-only result:', data);
            ...
            const transformedResult = {
                question: solution.question || textInput,
                answer: solution.answer || "Answer generated",
                confidence: solution.confidence || 95,
                steps: solution.steps || [],
                keyPoints: solution.key_points || ["AI-generated solution"],
                explanation: solution.explanation || solution.answer || "Detailed explanation provided"
            };
            ...
            console.error('Error in text-only submit:', error);
            displayToast("Failed to process text question. Please try again.", "error");
            setLoading(false);
            setStep(2);
        }
    };
    ...
    const testAPIConnection = async () => {
        try {
            ...
            if (response.ok) {
                ...
                displayToast("API connection successful", "success");
            } else {
                throw new Error(`Health check failed: ${response.status}`);
            }
        } catch (error) {
            console.error('API connection test failed:', error);
            displayToast("Cannot connect to API", "error");
        }
    };
    ...
    const processFile = async (file) => {
        return new Promise((resolve, reject) => {
            if (file.type === 'application/pdf') {
                ...
                resolve(`Please process this PDF file: ${file.name}. This is a PDF document that needs to be analyzed for assignment questions.`);
            } else if (file.type === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
                // For DOCX files, you'll need a DOCX parsing library
                resolve(`Please process this DOCX file: ${file.name}. This is a Word document that needs to be analyzed for assignment questions.`);
            } else if (file.type.startsWith('image/')) {
                // For images, convert to base64 and ask for OCR
                const reader = new FileReader();
                reader.onload = (e) => {
                    resolve(`Please analyze this image for text content and assignment questions. The image contains: [Image uploaded - ${file.name}]. Please extract any text or questions visible in this image.`);
                };
    ...
    const processImage = async (imageDataUrl) => {
        return `Please analyze this captured image for text content and assignment questions. This is a photo taken by the user that likely contains assignment questions or problems. Please extract any text or questions visible in this image and provide solutions.`;
    };
